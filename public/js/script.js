...
const API_REQUEST_URL = "http://localhost:11434/api/generate"; // Connexion locale à Ollama
...
const requestApiResponse = async (incomingMessageElement) => {
    ...
    try {
        const response = await fetch(API_REQUEST_URL, {
            ...
            body: JSON.stringify({
                model: "llama3",
                prompt: `Tu es un assistant multilingue qui aide les utilisateurs à réserver une table (nom, date, heure, nombre de personnes) ou à passer une commande (produit, quantité). Réponds de manière concise et polie.\n\nUtilisateur: ${currentUserMessage}\nAssistant:`,
                stream: false
            }),
        });

        const responseData = await response.json();
        if (!response.ok) throw new Error(responseData.error || "Erreur de génération avec Ollama.");

        const responseText = responseData?.response?.trim(); // Ollama renvoie "response"
        ...
        isGeneratingResponse = false;
        messageTextElement.innerText = "❗ Veuillez réessayer bientôt.";
